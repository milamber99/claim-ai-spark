...
export const AssessmentResults = ({
  ...
  return <div className="space-y-6">
      ...
      {/* Uploaded Photos */}
      {claimData.imageData && claimData.imageData.length > 0 && <div>
          <h3 className="text-lg font-semibold text-foreground mb-3">Submitted Photos</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {claimData.imageData.map((image, index) => <img key={index} src={image} alt={`Damage photo ${index + 1}`} className="w-full h-40 object-cover rounded-lg shadow-md" />)}
          </div>
        </div>}
          ...
          <div className="flex-1">
            {isEditing ? <div className="space-y-3">
                <div>
                  ...
                  <Select value={editedData.severity} onValueChange={(value: "minor" | "moderate" | "severe") => setEditedData({
                ...editedData,
                severity: value
              })}>
                ...
                <div>
                  ...
                  <Input value={editedData.estimatedCost} onChange={e => setEditedData({
                ...editedData,
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {currentData.damageAreas.map((area, index) => <Badge key={index} variant="secondary" className="px-3 py-1 flex items-center gap-2">
              {area}
              {isEditing && <button onClick={() => removeDamageArea(index)} className="hover:text-destructive" aria-label="Remove area">
                  <X className="h-3 w-3" />
                </button>}
            </Badge>)}
        </div>
        {isEditing && <div className="flex gap-2 mt-3">
            <Input value={newDamageArea} onChange={e => setNewDamageArea(e.target.value)} placeholder="Add new damage area" onKeyPress={e => e.key === "Enter" && addDamageArea()} />
      ...
      <div>
        ...
        <Card className="p-4 bg-muted/50">
          {isEditing ? <Textarea value={editedData.assessment} onChange={e => setEditedData({
          ...editedData,
          assessment: e.target.value
        })} rows={6} className="bg-background" /> : <p className="text-foreground leading-relaxed whitespace-pre-line">{currentData.assessment}</p>}
        </Card>
      </div>
      ...
      <div>
        ...
        <Card className="p-4 bg-card">
          <ol className="space-y-2">
            {currentData.nextSteps.map((step, index) => <li key={index} className="flex items-start gap-3">
                ...
                {isEditing ? <div className="flex-1 flex gap-2">
                    ...
                    <Button onClick={() => removeNextStep(index)} variant="ghost" size="sm" className="text-destructive hover:text-destructive">
                      <X className="h-4 w-4" />
                    </Button>
                  </div> : <span className="text-foreground pt-0.5">{step}</span>}
              </li>)}
          </ol>
          {isEditing && <Button onClick={addNextStep} variant="outline" size="sm" className="mt-3">
              Add Step
            </Button>}
        </Card>
      </div>
